...
Keepa のクエリではブランドを絞れないため。**`/rival-research` と共用**しており、
ライバル候補からも同じリストで弾く。

- 1688 に同款が無い: タカラトミー・オムロン・コールマン
- 日本の生活雑貨メーカー（同じ棚に並んでも中国輸入の競合にならない）: レック・ダルトン・
  山崎実業・山崎産業・無印良品・マーナ・アイリスオーヤマ・ニトリ・貝印・パール金属
- **誤爆しやすい短い綴りは英語表記を入れない。** `lec` は select / collect / electric に、
  `muji` も他の語に含まれうる
