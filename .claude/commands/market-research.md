...
3. **ポケモンカードはルートでは落とせない。** トレカは「ホビー」配下（`2189358051` コレクションカード・アクセサリ、`10345415051` トレーディングカードゲーム）にあり、ホビーごと除外するとプラモ・鉄道模型まで落ちる。**サブカテゴリ指定で除外する**
4. **化粧品はルートカテゴリ「ビューティー」(`52374051`) ごと除外している。** 薬機法で化粧品は輸入できないため。サブカテゴリ（スキンケア・メイクアップ等）単位ではなくルートで落としているので、メイクブラシ・パフのような雑貨も巻き添えで落ちる
5. **`perPage` は50未満だと400エラー**（`combination of perPage and page exeeds limit or is too small`）。1回のクエリで11トークン消費する
