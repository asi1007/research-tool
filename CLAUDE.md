...
画像検索による自動登録は `/find-supplier`。手順と落とし穴はスキル
`.agents/skills/find-supplier/SKILL.md` に集約してある。特に次の3点は知らないと必ず詰まる。
...
- **印の移動先タブは、対象ではなく「対象外」で決める**（`research_sheets.candidate_sheets`）。
  以前は `discovery_sheets()` を流用してタブ名から価格帯を読んでいたため、`自動調査◯◯円` を
  名乗らない **`優先` タブが定期実行の視界に入らず、`d` が31件溜まっていた**（2026-09-23）。
  候補タブは価格帯を増やすたびに増えるが、非候補タブ（`リリース` `調査済み` `サンプル発注管理`
  `テンプレ(新)` `idea` と移動先3つ）は増えない。**対象を数え上げると必ず漏れる**
...
**同じ ASIN の行が複数あるときは書かない。** どちらを読んだ行なのか決められないため、
`relocate_updates` は `ambiguous` として返し `fetch_products` はログに残して飛ばす。
ASIN 列には素の ASIN と商品URLが混在するので、**URL だけの行と素の ASIN 行が同じ商品を指す**ことがある
（2026-09-21 に `自動調査500円-700円` で実在）。**シートの並び替えは日常的に行われる**ので、
「最初に見つかった行へ書く」旧仕様では読んだ行と別の行を上書きしうる。

**読み取り時の行番号を書き込みに使ってはいけない。** 読み取りから書き込みまでの間に
行が挿入・削除・並び替えされると全件ずれる。2026-08-31 に `shorten_titles` の短縮名 52件が1行ずれた
...
- **工程どうしに依存は無い。** どれかが落ちても残りは実行し、最後に非ゼロで終える。
  途中で止めると後続が丸ごと欠測する
- **想定内の終了はジョブの失敗にしない**（`EXPECTED_EXIT_CODES`）。`find_supplier_auto` の
  **2（キャプチャ）と 3（1688 の遮断）**は相手側の事情で、こちらに直すものが無い。
  非ゼロで返すと Slack 通知が鳴り続け、本当の失敗が埋もれる。ログ（WARNING）には残して次の工程へ進む
- **`fetch_products --all` の対象タブはシートから引く。** 自動調査タブは名前が価格帯で決まるため
  固定リストに書くとリネームに追従できない。実際に 2026-08-30 のリネームから1年近く気づかれず、
  `候補` `リサーチ700円以下` `ハードル高い` を見に行って **WorksheetNotFound で exit 1**、
  一方で**肝心の自動調査タブを1つも見ていなかった**（2026-09-21 に定期実行へ組み込んで発覚）。
  自動調査タブ以外は `FIXED_SHEETS`。候補外・季節商品・保留は `drop_marked` の移動先なので対象にしない
...
- **有名ブランドは候補にしない。** `EXCLUDED_BRANDS` を市場調査と共用しており、レック・ダルトン・
  ...
  （市場調査側はブランド欄で照合する `EXCLUDED_MAKERS` も使う。ライバル調査はブランド欄を持たないので使わない。`/market-research` 参照）

## 仕入先の規格は『规格』列に残す

各仕入先リンクの直後に **『规格』列**（`SPEC_LOWEST` / `SPEC_BUY_OTHER1` / `SPEC_BUY_OTHER2`）がある。
`find_supplier.py write` に渡す候補 JSON の `spec` がここへ入る。

- **中国語の規格名をそのまま写す。** 訳すと 1688 側で探せなくなる
- **価格が取れなくても書く。** URL だけ入れる場合も、どの規格を見て判断したかを残す
- **2026-09-09 に追加した。** それ以前の行は規格が `title` に混ざっている

カード価格は別構成の値段であることが多く（`/find-supplier` の表を参照）、規格を残していないと
「どのサイズ・色で見積もったか」が後から追えない。
