...
### AB列とJ列は「バグに見えるが直さない」と決めた仕様

2026-08-22 にユーザーが判断した。**再提案しないこと。**

- **AB列 `販売数/FBA数` には書き込まない。** 改行のせいで完全一致に失敗するが、ヘッダー正規化は入れない
- **J列 `数量` に月間販売数（Keepa の `monthlySold`）を書く。** I列「金額」の隣で仕入数量欄に見えるが、これで正しい

実行ログに毎行出る `シートに無いため書き込まない列: ["販売数/FBA数"]` は**想定どおり**。

Python 側の `src/infrastructure/column_mapper.py` は `normalize_header()` で改行を除去しており、
コードだけ見ると GAS 側が未対応の不具合に見える。上の判断が優先する。
