...
### 現地価格は「最安サイズ」ではなく対応サイズの価格を使う

**検索結果カードに出る価格は、その商品の最安バリエーションの価格。**
サイズ違いで単価が数倍変わるため、カード価格をそのまま書くと原価を大幅に過小評価する。

2026-08-25 の実測（磁石、`offerId=674948577389`、全116サイズ、最安¥0.12〜最高¥62）:
...
Amazon商品名にサイズがある場合は、候補の商品ページ（`detail.1688.com/offer/{offerId}.html`）を
開き、`scripts/extract_variants.js` でサイズ表を取って該当サイズの価格を選ぶ。
`規格名 ¥価格` の並びを**行単位で**拾う。`F20*10*3mm` `E16银色` `G20 拉力（0-6kg）180°` と
表記が揺れるため、`\d+\*\d+\*\d+mm` のような形の決め打ちはできない。

サイズだけでなく**色と耐荷重も見る**。規格名に `拉力0-5.5kg` のように書かれていることが多い。

**2026-08-25 より前に書き込んだ47行はカード価格（最安サイズ）のまま。** ユーザーの判断で
遡及修正はしていない。これらの現地価格は下限値として扱う。
